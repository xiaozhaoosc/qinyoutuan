...
| E sshctl 落地集成 | ◐ 核心+编排✔（远端实机待测） | `sbproc.RemoteEnsureScript`（生成远端 root 脚本：写/比对 systemd unit + daemon-reload+enable+restart，打印 `ARGO_CHANGED`/`ARGO_HOST` 标记）+ `EnsureRemoteArgo`（注入式 SSH runner，Host 按 tag 缓存，供订阅 13 端口复用）+ `api.StartArgoSync`（跨服务器循环：有凭据且有 argo 入站的服务器，经 sshctl 跑脚本）。本地凭据未配/节点离线则记日志跳过。**跨平台注意**：脚本是 Linux bash，路径强制 `/` 拼接（filepath.Join 在 Windows 会出 `\`）。**未完**：需一台真实远端落地机做 SSH 实机联调 |
